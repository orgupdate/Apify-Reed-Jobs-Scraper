# 🚀 Reed Jobs Scraper – Scrape Reed Jobs to JSON, CSV & Excel

Scrape and aggregate job listings from **Reed** (reed.co.uk) search results – job title, company, location, salary, posting date and a direct apply link – and export them to JSON, CSV, Excel or an API in a single run.

[View on Apify Store](#)

## 📖 Overview

The **Reed Jobs Scraper** is a data-extraction tool that turns Reed job search results into structured data. Reed is the UK's largest independent job site, covering roles across every sector.
...
- **🔍 Full-Text Job Search** – Search Reed by keyword, job title or skill and walk every result page automatically.
...
```
https://mcp.apify.com/?tools=actors,docs,orgupdate/reed-jobs-scraper
```
...
With the server connected, your agent can search Reed listings by title and city, compare postings across employers, and pull salary and apply links into a single structured answer.
...
| `companyName` | String | No | Restrict results to a single employer (e.g. `"NHS"`). | - | Any company name |
...
Not every Reed listing includes every field – `salary`, `job_type` and exact `date` vary by posting – so handle optional fields gracefully in your integration.
...
Run this Actor from the Apify API with the official clients. Replace `orgupdate/reed-jobs-scraper` with the real Actor ID from the Actor page.
...
run = client.actor("orgupdate/reed-jobs-scraper").call(run_input=run_input)
...
const run = await client.actor('orgupdate/reed-jobs-scraper').call(input);
...
- **Find remote Reed jobs via MCP** – let an AI agent pull remote listings with company, source and direct apply links.
- **Track NHS's open roles** – monitor one employer's postings on Reed on a daily schedule.
- **Jobs posted in the last 3 days** – recent Reed listings filtered by `datePosted`, feeding a database.
- **Salary benchmark by city** – aggregate Reed pay ranges for one job title across multiple locations.

## 💡 Use Cases

- **Job boards & aggregators** – Populate a niche board with fresh Reed listings, no manual entry.
...
Some queries genuinely have few listings on Reed. Broaden the keyword (e.g. `"engineer"` instead of `"senior backend engineer"`), widen `datePosted`, and raise `pagesToFetch`.
...
Every row this Actor returns starts as a public listing on Reed (reed.co.uk). Employers and job boards publish those postings; this Actor reads the search results and parses them into fields. This is an independent tool. It is **not** affiliated with, endorsed by, or operated by Reed, and it does not use a private Reed API.

### How is this different from searching Reed by hand?

Browsing Reed gives you one page of cards at a time and makes you keep loading more. This Actor walks the pagination for you and returns the whole set in one run, already parsed into `job_title`, `company_name`, `location`, `salary`, `date` and `URL` – fields you can sort, filter and export. An afternoon of scrolling and copy-paste becomes a run of a few minutes.

### Does Reed have a public jobs API?
