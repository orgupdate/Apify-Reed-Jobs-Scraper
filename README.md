...
The most efficient way to extract job listings directly from Reed Jobs search results.

[View on Apify Store](#)

## 📖 Overview

The **Reed Jobs Scraper** is a powerful data extraction tool designed to aggregate job listings from the Reed Jobs search engine.
...
- **🌍 Multi-Source Aggregation** – Access listings from thousands of job boards via a single Reed search query.
...
|-----------|------|----------|-------------|---------|--------------|
| `countryName` | String | No | The country context for the Reed Search domain | "all" | "usa", "uk", "india", etc. |
...
| `companyName` | String | No | Filter listings to a specific employer | - | Any company name (e.g., "Reed", "Microsoft") |
...
- **posted_via:** The original platform where Reed found the job (e.g., Monster, Greenhouse, LinkedIn).
...
- **Reed Sheets:** Automatically save new job listings into a spreadsheet for analysis.
